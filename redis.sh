...
dnf module enable redis:remi-6.2 -y >\tmp\roboshop.log  &>>${log}

echo -e "\e[33m >>>>>> install redis <<<<<< \e[0m"
dnf install redis -y >\tmp\roboshop.log  &>>${log}
# update listen address /etc/redis.conf & vim /etc/redis/redis.conf
echo -e "\e[34m >>>>>> enable redis  <<<<<< \e[0m"
systemctl enable redis >\tmp\roboshop.log  &>>${log}

echo -e "\e[35m >>>>>> restart redis <<<<<< \e[0m"
systemctl restart redis >\tmp\roboshop.log  &>>${log}